...
const COOKIE_NAME = "amyth_wallet_session";
const SIG_COOKIE_NAME = "amyth_wallet_session_sig";
const NONCE_TTL_SEC = 5 * 60; // 5 minutes
const NONCE_MAX_OUTSTANDING = Number(process.env.AMYTH_NONCE_MAX_OUTSTANDING || 5); // allow up to N concurrent device nonces per wallet
const NONCE_RL_WINDOW_SEC = Number(process.env.AMYTH_NONCE_RL_WINDOW_SEC || 60);
const NONCE_RL_MAX_PER_WALLET = Number(process.env.AMYTH_NONCE_RL_MAX_PER_WALLET || 10); // per wallet per window
const NONCE_RL_MAX_PER_IP = Number(process.env.AMYTH_NONCE_RL_MAX_PER_IP || 30); // per ip per window
export const SESSION_TTL_SEC = Number(
  process.env.AMYTH_WALLET_SESSION_TTL_SEC || 60 * 60 * 24 * 365
);
const COOKIE_DOMAIN = (process.env.AMYTH_COOKIE_DOMAIN || "").trim(); // e.g. "amyth.trade"

const SIGN_APP_NAME = (process.env.AMYTH_SIGN_APP_NAME || "Amyth").trim();
const SIGN_DOMAIN = (process.env.AMYTH_SIGN_DOMAIN || "amyth.trade")
  .replace(/^https?:\/\//, "")
  .trim();
...
async function rateLimitIssueNonce(opts: {
  wallet: string;
  ip?: string | null;
  ...
  const wKey = `amyth:auth:rl:nonce:wallet:${wallet}`;

  try {
    const wCount = await r.incr(wKey);
    if (wCount === 1) {
      try {
        await r.expire(wKey, NONCE_RL_WINDOW_SEC);
      } catch {
        /* ignore */
      }
    }
    if (wCount > NONCE_RL_MAX_PER_WALLET) {
      return { ok: false, error: "rate_limited" };
    }

    if (ip) {
      const ipKey = `amyth:auth:rl:nonce:ip:${ip}`;
      const ipCount = await r.incr(ipKey);
      if (ipCount === 1) {
        try {
          await r.expire(ipKey, NONCE_RL_WINDOW_SEC);
        } catch {
          /* ignore */
        }
      }
      if (ipCount > NONCE_RL_MAX_PER_IP) {
        return { ok: false, error: "rate_limited" };
      }
    }
...
export async function issueNonce(
  ...
  const nonceKey = `amyth:auth:nonce:${wallet}:${nonce}`;
  const idxKey = `amyth:auth:nonce_idx:${wallet}`;

  try {
    await r.set(nonceKey, String(ts), { ex: NONCE_TTL_SEC });

    // Index + cap enforcement (best-effort; nonce validity is still secure without index)
    try {
      await r.zadd(idxKey, { score: ts, member: nonce });
      try {
        await r.expire(idxKey, NONCE_TTL_SEC + 60);
      } catch {
        // ignore
      }

      const card = await r.zcard(idxKey);
      const max = Math.max(1, NONCE_MAX_OUTSTANDING || 5);
      if (card > max) {
        const excess = card - max;
        const old = await r.zrange<string[]>(idxKey, 0, excess - 1);
        if (old && old.length) {
          try {
            await r.zrem(idxKey, ...old);
          } catch {
            // ignore
          }
          for (const n of old) {
            try {
              await r.del(`amyth:auth:nonce:${wallet}:${n}`);
            } catch {
              // ignore
            }
          }
...
export async function consumeNonce(wallet: string, nonce: string): Promise<boolean> {
  ...
  const idxKey = `amyth:auth:nonce_idx:${wallet}`;
  try {
    const stored = await r.get<string | null>(nonceKey);
    if (!stored) return false;

    // Consume nonce (one-time use)
    try {
      await r.del(nonceKey);
    } catch {
      // ignore
    }
    // Best-effort remove from index
    try {
      await r.zrem(idxKey, nonce);
    } catch {
      // ignore
    }
    return true;
...
// (amyth_wallet_session_sig) is set and required when reading the session.
...
export async function getSessionWallet(req: Request): Promise<string | null> {
  try {
    ...
    // amyth_wallet_session cookies in play (different domains / historical formats),
    ...
    // a matching amyth_wallet_session_sig cookie.
